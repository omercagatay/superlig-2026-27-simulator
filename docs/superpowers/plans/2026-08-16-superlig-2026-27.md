...
Run: `grep -rn "wc2026_sim\|wc2026-sim" --include=*.rs --include=*.toml --include=Dockerfile --include=.env.example --include=*.yml .`

Update each hit to `superlig_sim` / `superlig-sim`. Known hits as of the last
check: `src/main.rs` (the `use` block and the tracing directive
`"wc2026_sim=info"`), `examples/fit_dc.rs` (two `use` lines and
`with_env_filter("wc2026_sim=info")`), `Dockerfile` (the dummy-layer `rm -rf`
globs and the binary copy path), `.env.example` (`RUST_LOG=wc2026_sim=info`),
plus `tests/calibration.rs` from Task 14.

The crate name appears inside `RUST_LOG` and `EnvFilter` *values*, not just in
paths. Missing those compiles fine and silently logs nothing — which is why the
grep must not be restricted to `*.rs`/`*.toml`.
...
Update any comment naming the World Cup, and change the log filter value to
`RUST_LOG=superlig_sim=info` (Step 2). `KIMI_API_KEY`, `PORT`,
`ENSEMBLE_WEIGHTS`, `LIVE_REFRESH_MINUTES` and `TRUST_PROXY` keep their
meanings and values.
...
### Task 19: Deploy to Railway as a new project

**Do this only after every item in the Verification Checklist below passes.**

**The existing `worldcup-2026-simulator` project must not be touched.** The user
was explicit: deploy as a *new* project. The World Cup app stays live at
`https://worldcup-2026-simulator-production.up.railway.app`.

Current state of the repo's Railway link (verified 2026-08-16):

- Workspace `Çağatay's Projects`
- The CLI is linked to project `worldcup-2026-simulator`
  (`1d35df6d-2930-44d1-8551-01d43d84e706`), environment `production`, with no
  service selected.
- **This link must be repointed before deploying.** A bare `railway up` from
  this directory targets the World Cup project.

- [ ] **Step 1: Create the new project and link it**

```bash
cd /home/cagatay/Desktop/wc2026-sim
railway init --name superlig-sim
railway status
```

Expected: `railway status` now reports project `superlig-sim`, not
`worldcup-2026-simulator`. Do not proceed until it does — this is the guard
against overwriting the World Cup deploy.

- [ ] **Step 2: Set the service variables**

The new project starts with no application variables. Set:

```bash
railway variables --set "RUST_LOG=superlig_sim=info" --set "TRUST_PROXY=1" \
  --set "ENSEMBLE_WEIGHTS=0.5,0.3,0.2" --set "LIVE_REFRESH_MINUTES=30"
```

Leave `PORT` unset — Railway injects it.

`KIMI_API_KEY` also needs setting or `/api/scenario` returns an error (the rest
of the app works without it). The key already exists on the old project; read it
with `railway variables --service worldcup-2026-simulator` while still linked
there in Step 1, or ask the user. **Do not write the key into any file in the
repo** — pass it to `railway variables --set` directly.
...
change — a stale `target/release/wc2026-sim` copy path fails here and only here,
never in `cargo build`.
...
Record the URL it prints; that is the deliverable to hand back to the user.

- [ ] **Step 5: Verify the live deployment**

```bash
BASE=<url from step 4>
...
If health fails, `railway logs` shows why.

- [ ] **Step 6: Confirm the World Cup deploy is untouched**

```bash
curl -s -o /dev/null -w '%{http_code}\n' \
  https://worldcup-2026-simulator-production.up.railway.app/api/health
```

Expected: `200`. If this is anything else, the wrong project was deployed to.
...
Then, and only then, Task 19 deploys to Railway — as a **new** `superlig-sim`
project. The existing `worldcup-2026-simulator` deploy stays live and untouched.
